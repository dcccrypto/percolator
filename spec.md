# Risk Engine Spec (Source of Truth) — v16.8.10 Realizable Full Shared Cross-Margin
...
consume - residual cure:        counterparty_backing_principal -= X ;
...
Forfeit-move typing under requirement 13: a transition whose ONLY credit is the derived closing class (`junior_residual_pool`) — i.e. the impair/expiry forfeits — does not emit a `TokenValueFlowProof` credit entry, because the pool has no independent counter and a flow credit to a derived class would be double bookkeeping. Such moves are instead proven by their `ReservationEncumbranceProof`, the lockstepped O(1) aggregate delta on `counterparty_backing_principal`, and the residual identity re-checked at the same transition (the impair/expiry witnesses assert all three). Every move with a NON-derived destination (reserve, realize, cure, external deposit/withdrawal) still requires its balanced `TokenValueFlowProof` as listed above.
