# Risk Engine Spec (Source of Truth) — v12.19.53
...
This revision supersedes v12.19.52. It preserves the v12.19.52 economics and fixes one remaining bounded-catchup precision edge. Loss-stale bounded catchup blocks user-directed position changes, liquidation/ADL OI changes, and exposure removal before the remaining historical mark/funding segments have been applied, but it MUST still allow canonical live-touch settlement to clear a position whose effective quantity has already floored to zero at the current loss-accrued segment. That floor-to-zero clear is not a user exit: it settles through the current segment, mutates no global OI, adds only potential dust unless an exact side proof certifies dust, and prevents zero-effective stored basis from receiving future segment PnL. All h-max, bounded-catchup, permissionless recovery, incremental keeper progress, loss-stale reset cleanup, bankruptcy h-lock, and prior safety/liveness requirements remain in §§0–10.
...
32. Public trading during an h-max-lock sweep MUST be conservative but live once the market is loss-current. A risk-increasing trade while h-max lock is active and `slot_last == current_slot` MUST use `Eq_trade_open_no_pos_i` and any required effective-price/raw-target dual-price checks; if those checks and all ordinary side-mode, OI, position, fee-current, and margin checks pass, the trade MUST NOT be rejected solely because the h-max envelope has not yet cleared. Risk-reducing and flat-result trades likewise MUST NOT be rejected solely because h-max lock is active. This rule does not override the loss-stale no-position-change rule in §0.38.
33. Post-principal negative PnL is a bankruptcy signal in live mode. If a live authoritative touch, no-accrual flat-negative cleanup, deposit loss-settlement, or any realized-loss path reduces an account's protected principal as far as available and the account still has `PNL_i < 0`, the instruction MUST start or restart the bankruptcy h-max lock before commit, even if the account is flat, remains nonflat, or the negative tail will be handled later by liquidation/ADL, insurance, uninsured loss, or another explicit loss path. Any live absorption of a post-principal negative tail through insurance/uninsured loss is also a bankruptcy signal. A live public instruction MUST NOT commit a newly discovered or newly worsened post-principal negative tail, or clear such a tail through insurance/uninsured loss, while leaving winners on ordinary `admit_h_min` / positive-credit lanes.
34. Same-instruction h-max activation is retroactive for the instruction's own positive-PnL usability mutations and positive-credit approvals. A permissionless keeper or caller MUST NOT be able to admit or mature a winning account at `admit_h_min`, release or accelerate an existing reserve, convert released PnL, auto-convert positive PnL, approve a risk increase with positive-PnL credit, and then trigger bankruptcy h-max later in the same atomic instruction. If h-max activates or restarts before commit, the instruction MUST either recompute those mutations/approvals under h-max/zero-positive-credit lanes, retroactively reclassify the instruction-local positive PnL back into h-max reserve and undo any same-instruction conversion, or fail before commit.
35. A public market with `cfg_public_liveness_profile == CrankForward` MUST be crank-forward by construction or by canonical permissionless recovery. Such a wrapper MUST NOT deploy a price scale/configuration where exposed live catchup can enter a permanent `max_delta == 0` dead zone unless it also exposes `permissionless_recovery_resolve` in §8.8, or a stricter public recovery path with the same oracle-authentication, h-max, payout-readiness, and anti-extraction rules. A recovery caller MUST NOT be able to force recovery by omitting/corrupting proofs, naming an unproven reason, or choosing invalid segment inputs; the recovery reason and recovery price must be proven from authenticated state and deterministic market policy. CrankForward profiles SHOULD instead enforce the progress-floor invariant in §1.7 so every exposed bounded catchup segment can move by at least one price unit whenever target and effective price differ.
36. A CrankForward public market with stale exposed state MUST have permissionless bounded catchup. If `now_slot - slot_last > cfg_max_accrual_dt_slots`, a public keeper path MUST be able to advance one bounded accrual segment and perform protective progress instead of requiring privileged recovery or a one-shot full catchup. Repeated bounded catchup cranks MUST be able to move `slot_last`, K/F, h-max state, and the sweep cursor forward until ordinary live operation resumes. If K/F future-headroom or representability prevents the next bounded segment, a CrankForward profile MUST route to `permissionless_recovery_resolve` in §8.8, or a stricter public recovery path; privileged-only recovery is not sufficient.
37. Candidate and account-data packing limits MUST produce bounded partial progress, not compute or wire-format deadlock. Public wrappers MUST bound candidate entries inspected as well as successful revalidations, MUST expose a cursor-priority or cursor-only crank composition, and MUST design authenticated account storage/proofs so an honest keeper can permissionlessly supply the current Phase 2 cursor account whenever that slot is materialized.
38. Bounded stale-catchup MUST preserve loss-senior fee ordering. If `slot_last < current_slot` because the market has only partially accrued to the authenticated instruction slot, recurring or explicit fees drawn from a nonflat or potentially stale account's capital MUST NOT be anchored past `slot_last` until the account's remaining mark/funding/ADL losses through the requested fee interval have been settled or the account is proven flat/current. A keeper may still liquidate, settle, or sweep such an account using the loss-safe fee anchor; user payout/free and extraction-sensitive operations requiring full fee-current state MUST wait, reject, or route through recovery until loss accrual is caught up.
39. Bounded stale-catchup MUST preserve historical exposure. If `slot_last < current_slot` because bounded catchup is incomplete and the remaining interval may contain equity-active mark/funding, the market is loss-stale. While loss-stale, the effective h-max lock is active for positive-PnL usability: fresh positive PnL MUST use `admit_h_max`, reserve release/acceleration/conversion/auto-conversion MUST be paused, flat touched-account auto-conversion MUST be skipped, and public withdrawal/risk-increasing approvals MUST use zero-positive-credit lanes. In addition, no instruction may let an account change represented current-epoch exposure, open fresh exposure, execute a trade, close a position, withdraw/free a nonflat account, or execute ordinary liquidation/ADL that changes OI until either `slot_last == current_slot` or terminal recovery enters `Resolved`. This prohibition does not block two canonical protocol-visible cleanup actions required for crank progress: (a) §5.2 live-touch settlement that clears basis when the account's effective quantity has already floored to zero at the current loss-accrued segment, and (b) §5.5 end-of-instruction certified-phantom/orphan reset cleanup. The §5.2 floor-to-zero clear MUST settle through the current segment first, MUST NOT mutate global OI, MUST add only potential dust unless an exact side proof certifies dust, and MUST NOT be used as a user-directed close, withdrawal, payout, or liquidation substitute. Permissionless keeper cranks, non-position-changing account touches/settlements, §5.2 floor-to-zero cleanup, and §5.5 reset cleanup continue so catchup can complete. This prevents positions from escaping still-unaccrued historical mark/funding while also preventing zero-effective stored basis from receiving future segment PnL. The loss-stale lock clears automatically when `slot_last == current_slot` or terminal recovery enters `Resolved`.
...
`stress_gate_active(ctx)` / `hmax_lock_active(ctx)` is true iff either `threshold_stress_gate_active(ctx)` is true or `bankruptcy_hmax_lock_active == true`.

`loss_stale_positive_pnl_lock_active(ctx)` is true in live mode when `slot_last < current_slot` because bounded catchup is incomplete and the remaining interval has not been proven free of equity-active mark/funding. This is a derived lock, not a persistent reconciliation envelope: it does not by itself advance or clear `stress_envelope_remaining_indices`, and it clears automatically when loss accrual catches up (`slot_last == current_slot`) or terminal recovery enters `Resolved`.

`hmax_effective_active(ctx)` is true iff `stress_gate_active(ctx)` is true, the instruction-local h-max candidate described below is active, or `loss_stale_positive_pnl_lock_active(ctx)` is true. Endpoint-local positive-PnL usability and approval decisions MUST use `hmax_effective_active(ctx)` unless a rule explicitly refers only to persistent state. When the effective h-max lock is active, scheduled reserve release is paused and existing released positive PnL MUST NOT be auto-converted, explicitly converted, or used as positive credit for public withdrawal or risk-increasing approval. If the only active h-max source is an ordinary stress/bankruptcy envelope and `slot_last == current_slot`, this lock is not a blanket halt for fee-current settlements, liquidations, keeper cranks, or trades that pass conservative lanes. If `loss_stale_positive_pnl_lock_active(ctx)` is true, §0.38 and §8.7.1 additionally forbid position-changing trades, closes, and liquidation/ADL execution until loss catchup completes or recovery resolves.

`trigger_bankruptcy_hmax_lock(ctx)` is called by any live bankruptcy event, including a liquidation account loss beyond principal (`D > 0`), a live post-principal negative tail discovered by `settle_negative_pnl_from_principal`, a live post-principal loss absorbed through insurance/uninsured loss, or an equivalent protocol-loss path. It computes candidates before commit:
...
`settle_negative_pnl_from_principal(i[, ctx])` is the canonical loss-cleanup helper. If `PNL_i < 0`, it computes `pay = min(C_i, abs(PNL_i))`, reduces protected capital through `set_capital(i, C_i - pay)`, and calls `set_pnl(i, PNL_i + pay, NoPositiveIncreaseAllowed)`. If live mode remains with `PNL_i < 0` after this principal settlement, the remaining negative tail is a bankruptcy signal: the caller MUST call `trigger_bankruptcy_hmax_lock(ctx)` before commit, or, if no live h-max context is available, reject or route through a live context that can start the h-max lock. This applies equally to standard live touches, no-accrual flat-negative cleanup, deposit loss-settlement, and helper-equivalent paths. Any remaining negative PnL may be cleared to zero with `set_pnl(..., NoPositiveIncreaseAllowed)` only after the same amount has been absorbed through `absorb_protocol_loss` or an exactly specified loss path, and any live such absorption MUST also satisfy the bankruptcy h-max rule in §4.6.
...
Loss-senior live fee anchoring is stricter than the raw upper bound. If `market_mode == Live`, `slot_last < current_slot`, and the account is nonflat or not proven flat/current with no remaining unsettled side effects through the requested interval, the maximum fee anchor for any fee drawn from that account's capital is `slot_last`. The caller may sync only to `min(anchor, slot_last)` and then continue keeper settlement/liquidation under that loss-safe fee state. It MUST NOT charge the same account to `current_slot` or `authenticated_now_slot` before the remaining bounded catchup losses have been accrued and settled for that account, because doing so would make fees senior to still-unaccrued mark/funding/ADL losses. Accounts proven flat/current may sync to `current_slot`; resolved accounts use `resolved_slot`. On live nonflat/current-state paths, recurring fee sync MUST occur after authoritative touch/loss settlement and before health-sensitive checks, approvals, conversions, liquidations, or payouts, using this loss-safe anchor.
...
`absorb_protocol_loss(loss_abs[, ctx])` calls `use_insurance_buffer` and records only the returned nonzero remainder. In live mode, any call with `loss_abs > 0` after the account's principal has been exhausted is a bankruptcy event and MUST call `trigger_bankruptcy_hmax_lock(ctx)` in the same atomic instruction before commit. A live caller that has no h-max-capable context MUST reject or reroute before absorbing the loss; it MUST NOT consume insurance, record uninsured loss, or clear the negative PnL tail in a no-context live path. Resolved-mode loss absorption does not affect live h-lock state.
...
During bounded stale-catchup, `schedule_end_of_instruction_resets` MUST NOT run ordinary user-directed position exits or liquidation/ADL, but it MAY and MUST run the canonical cleanup branches in this section when their proofs hold. These cleanup branches are explicit reset/recovery mechanics: they may clear certified phantom or orphan OI, set pending reset flags, or begin full drain resets, but they MUST NOT be treated as permission for trades, nonflat withdrawals, account closes, or arbitrary OI mutation while loss-stale.
...
6. settles negative PnL from principal through `settle_negative_pnl_from_principal(i, ctx)`, so any remaining live post-principal negative tail starts/restarts bankruptcy h-max before commit;
7. if now authoritative flat and still negative, calls `absorb_protocol_loss(..., ctx)` so bankruptcy h-max starts/restarts before the negative tail is cleared, then sets PnL to zero;
...
`deposit(i, amount, now_slot)` may materialize missing `i` only if `amount > 0`. It computes the candidate vault inflow with checked arithmetic and requires `V + amount <= MAX_VAULT_TVL` before any materialization or capital write. It then increases `V`, increases protected capital through `set_capital(i, C_i + amount)` using the same candidate state, and settles already-realized losses from principal. If the deposit is insufficient and live `PNL_i < 0` remains after principal settlement, the operation MUST start/restart bankruptcy h-max with a no-accrual live h-max context, or reject/reroute before commit. It MUST NOT absorb flat negative loss through insurance on the deposit path, and it sweeps fee debt only if the account is flat and nonnegative.
...
`settle_flat_negative_pnl(i, now_slot[, fee_rate])` is no-accrual, requires an authoritatively flat account with `basis_pos_q_i = 0` and no reserve, and settles losses from principal first. If negative PnL remains after principal settlement, the operation MUST initialize a no-accrual live h-max context, call `trigger_bankruptcy_hmax_lock(ctx)`, absorb the remaining negative PnL through insurance/uninsured loss, and set PnL to zero; otherwise it MUST reject/reroute before commit. It then syncs fees only after the loss and h-max handling is complete. This path MUST NOT be used as an insurance-consumption shortcut that bypasses bankruptcy h-max.
...
`execute_trade(a,b, ..., size_q, exec_price)` requires distinct materialized accounts, valid execution price, positive size, computed `trade_notional <= MAX_ACCOUNT_NOTIONAL`, standard live lifecycle, and loss-current market state (`slot_last == current_slot`) after any required accrual in the same instruction. A trade MUST NOT execute while bounded catchup remains incomplete, because attaching or clearing exposure before the remaining historical mark/funding segments are applied would let accounts receive or avoid PnL for time intervals during which the candidate exposure did not exist.
...
`liquidate(i, ..., policy)` runs the standard live lifecycle and requires loss-current market state (`slot_last == current_slot`) before any liquidation execution that changes OI. Its endpoint logic touches the account first, settles A/K/F effects and negative PnL from principal, syncs recurring fees if enabled, requires liquidation eligibility on the fee-current authoritative state, and executes `FullClose` or `ExactPartial(q_close_q)`. During loss-stale bounded catchup, keepers may touch/revalidate candidates but MUST NOT execute liquidation/ADL quantity changes until catchup completes or terminal recovery resolves. The standard lifecycle performs the single required finalization, reset scheduling/finalization, and conservation check; implementations MUST NOT accidentally run those final steps twice.

### 8.7 Keeper crank: incremental model

`keeper_crank(authenticated_now_slot, oracle_price, funding_rate, admit_h_min, admit_h_max, threshold_opt, ordered_candidates[], max_candidate_inspections, max_revalidations, rr_touch_limit, rr_scan_limit[, fee_fn])` is live-only. It accrues exactly once before both phases in ordinary mode, or exactly one bounded segment in stale-catchup mode as specified in §8.7.1. In §8.7, `authenticated_now_slot` is the runtime/clock slot for the top-level instruction. If §8.7.1 bounded stale-catchup uses a smaller `segment_slot`, `authenticated_now_slot` controls Phase 2 wrap eligibility, `last_sweep_generation_advance_slot`, current-slot ownership, and h-max/stress start-slot comparisons; loss-senior fee anchors for nonflat or potentially stale accounts are capped by the last loss-accrued `slot_last` as specified in §4.5.
...
`sweep_generation` advancement is slot-rate-limited by `authenticated_now_slot`, not by any stale-catchup `segment_slot`. A cursor wrap may increment `sweep_generation` at most once per authenticated instruction slot. The market stores `last_sweep_generation_advance_slot`. A wrap may occur only when:

```text
last_sweep_generation_advance_slot == None || authenticated_now_slot > last_sweep_generation_advance_slot
```

A wrap that occurs in the same authenticated instruction slot as `stress_envelope_start_slot` is allowed only if the implementation is otherwise allowed to wrap that slot, but it is not an eligible post-event generation advance and does not help clear h-max lock. Same-instruction Phase 2 inspections after nonzero stress consumption or bankruptcy h-lock activation also do not reduce `stress_envelope_remaining_indices`. If accrual, a stale-catchup segment, Phase 1, or Phase 2 starts or restarts the reconciliation envelope during the current instruction, all Phase 2 progress from that same instruction is ineligible for the newly started envelope and any previously accumulated `stress_counted_inspected` candidate MUST be discarded before commit.
...
while inspected < scan_cap && touched < rr_touch_limit:
    if i == sweep_limit - 1 && !wrap_allowed:
        break
...
- Repeated same-slot cranks cannot advance `sweep_generation` more than once, cannot repeatedly wrap through the boundary, and cannot clear stress by repeated same-slot wraps.
...
authenticated_now_slot:
    owns public slot validation, current_slot advancement, Phase 2 cursor/wrap
    eligibility, last_sweep_generation_advance_slot, protective-progress
    accounting, and h-max/stress start-slot comparisons

slot_last / segment_slot:
    owns the last loss-accrued time. For nonflat or potentially stale accounts,
    this is also the maximum loss-safe live fee anchor until catchup completes.
    Segment time owns only this segment's K/F/price/fund_px_last/slot_last
    candidate and numeric segment_dt used for price/funding envelopes
```
...
If `slot_last < current_slot` after the segment, the market is still loss-stale for the remaining interval. Phase 1/Phase 2 may continue to touch, revalidate, and settle accounts. They MUST NOT execute liquidation, ADL quantity changes, trades, account close, nonzero attach, user-directed close/clear, or any OI-changing local operation until catchup completes or terminal recovery resolves. The sole local clear permitted during loss-stale touch is the canonical §5.2 floor-to-zero settlement clear: if an account's effective quantity is already zero after settlement through the current segment, the implementation MUST clear that basis so it cannot accrue later segment PnL, MUST NOT mutate global OI, and MUST add only potential dust unless an exact side proof certifies dust. The loss-stale positive-PnL lock in §4.3 MUST be treated as active until catchup completes or terminal recovery resolves. Fresh positive PnL touched during this interval is admitted at `admit_h_max`; reserve release, reserve acceleration, conversion, and flat touched-account auto-conversion are skipped; public withdrawal and risk-increasing approvals use zero-positive-credit lanes. Any fee drawn from a nonflat or potentially stale account's capital MUST use the §4.5 loss-safe fee anchor `<= slot_last`. The crank MUST NOT make that account fee-current to `authenticated_now_slot` until the remaining accrual segments affecting that account have been applied, the account is proven flat/current, or the market enters terminal recovery. User payout/free, trade, liquidation execution, or other extraction-sensitive/risk-changing actions requiring full loss-current state through `authenticated_now_slot` MUST reject/defer while this loss-stale interval remains.
...
Bounded stale-catchup is a liveness path, not a payout or historical-exposure shortcut: while `slot_last < authenticated_now_slot`, target/effective divergence, h-max, reserve-release, positive-PnL, zero-positive-credit, and loss-stale no-position-change restrictions remain in force.
...
14. If recurring fees are enabled, wrappers MUST sync fee-current state after authoritative touch/loss settlement and before health-sensitive checks, reclaim checks, and resolved terminal close, and MUST use `resolved_slot` on resolved markets. During bounded stale-catchup, a wrapper MUST cap capital-drawn fee anchors for nonflat or potentially stale accounts at the loss-safe `slot_last` until catchup or recovery makes the account current; it MUST NOT charge fees to `authenticated_now_slot` ahead of still-unaccrued losses. A wrapper MUST NOT charge fees from a stale/nonflat account's capital before settling that account's current losses, and MUST NOT allow `force_close_resolved` to return `ProgressOnly`, forgive fee debt, pay capital, or free a slot before the resolved fee sync has run.
14a. During bounded stale-catchup with `slot_last < current_slot`, wrappers MUST enforce the loss-stale lock: no reserve release/acceleration, conversion, auto-conversion, positive-PnL-credit approval, trade execution, nonflat withdrawal/close/free, or liquidation/ADL execution that changes OI may be exposed until catchup completes or terminal recovery resolves. This lock MUST NOT be implemented as a blanket keeper or non-position-changing settlement halt; bounded catchup, account touch/revalidation, and loss-safe settlement continue.
...
26. Public wrappers MUST NOT implement stress-sweep blanket trade halts once the market is loss-current. During active stress or incomplete post-event sweep coverage with `slot_last == current_slot`, they MUST keep at least one public trade path live for orders that pass no-positive-credit margin, target/effective dual-price checks when relevant, side-mode gating, OI/position limits, fee-current checks, and authorization. During loss-stale bounded catchup, trade execution is paused only until catchup completes or recovery resolves.
...
56. h-max-lock sweep nonblocking liveness: while `hmax_effective_active(ctx)` / `hmax_lock_active(ctx)` remains true for multiple cranks and new positive PnL is h-locked at `admit_h_max`, keeper cranks continue to accrue and make bounded Phase 1/Phase 2 progress. When `slot_last == current_slot`, risk-reducing trades proceed, and risk-increasing trades proceed when they pass `Eq_trade_open_no_pos_i`, dual-price checks when target/effective prices diverge, and all ordinary side-mode/OI/position/fee-current margin checks. Tests reject wrappers that halt all loss-current trading or roll back otherwise valid cranks solely because the post-event envelope is incomplete.
57. `admit_h_min = 0` immediate-release policy: when no h-max lock is active and `Residual > 0`, fresh positive PnL and outstanding reserves may mature immediately even when `PNL_matured_pos_tot + fresh_or_reserved_positive_pnl > Residual`; payouts/conversions remain haircut-bounded and fair exits are not guaranteed. If `Residual == 0` or after any live bankruptcy event, including a post-principal negative tail discovered before liquidation/ADL execution, a no-accrual flat-negative cleanup, a deposit loss-settlement that leaves negative PnL, or live insurance/uninsured absorption after principal exhaustion, the same market forces `admit_h_max`, pauses release/conversion, uses zero-positive-credit approval lanes, and clears only after the authenticated post-event envelope and eligible generation advance.
...
71. recurring-fee sync during bounded catchup: Phase 1/Phase 2 fee-current checks use the loss-safe `slot_last` anchor for nonflat or potentially stale accounts while remaining accrual segments exist, fees are not synced to `authenticated_now_slot` ahead of still-unaccrued losses, flat/current accounts may sync to current time, and user payout/free requiring full fee currentness rejects or defers until catchup or recovery makes the account fully current;
72. loss-stale historical-exposure lock during bounded catchup: when `slot_last < current_slot` after a bounded segment, fresh positive PnL is admitted at `admit_h_max`, existing reserves do not release or accelerate, flat touched-account auto-conversion is skipped, manual conversion and extraction-sensitive close/withdraw paths reject or use zero-positive-credit lanes, and trades, nonflat closes, and liquidation/ADL execution that changes OI are rejected until catchup completes or recovery resolves. Keeper bounded catchup, account touch/revalidation, and non-position-changing loss-safe settlement still make partial progress; and
73. post-principal negative-tail h-max: any live account whose authoritative settlement exhausts `C_i` and leaves `PNL_i < 0` starts/restarts bankruptcy h-max before commit, whether the account is nonflat, flat, discovered by standard touch, discovered by no-accrual flat-negative cleanup, or discovered by deposit loss-settlement. Live insurance/uninsured absorption of a post-principal negative tail also starts/restarts h-max before clearing the PnL. Tests reject any no-context live cleanup path that consumes insurance or clears the tail while winners remain eligible for ordinary `admit_h_min`, conversion, auto-conversion, withdrawal with positive-PnL credit, or trade approval with positive-PnL credit in the same or later pre-envelope-clear state; and
74. loss-stale cleanup exceptions: during bounded catchup with `slot_last < current_slot`, tests show ordinary trades, closes, withdrawals, nonzero attach, user-directed clear, and liquidation/ADL OI changes remain blocked, while (a) canonical §5.2 floor-to-zero settlement clears zero-effective basis without mutating global OI and adds only potential dust absent exact certification, and (b) canonical §5.5 certified-phantom and orphan-exposure reset cleanup branches still run when their proofs hold. These cleanup paths keep h-max/zero-positive-credit restrictions active and do not allow user-directed exposure escape.
