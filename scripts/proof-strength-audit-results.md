...
| `proof_permissionless_progress_resolved_mode_ignores_account_hint_on_prod_code` | 21.68s | PASS | Dispatcher resolved branch ignores symbolic non-cursor account hints, closes through the resolved cursor, strictly reduces public progress rank, and does not materialize the hinted slot. |
...
| `proof_permissionless_account_b_progress_reduces_hinted_account_b_rank_on_prod_code` | 217.25s | PASS | Production permissionless account-B progress branch validates, touches, finalizes, strictly reduces the hinted account's B rank, preserves market clock/price, and does not spend insurance. |
...
against the current 410-harness inventory, then rerun the static strength /
