...
use percolator::v13::{
    LiquidationRequestV13, MarketGroupV13, PermissionlessCrankActionV13,
    PermissionlessCrankRequestV13, PortfolioAccountV13, ProvenanceHeaderV13, TradeRequestV13,
...
fn fuzz_group() -> MarketGroupV13 {
    let (market, _, _, _) = ids();
    let mut cfg = V13Config::public_user_fund(1, 0, 10);
    cfg.max_trading_fee_bps = 10;
    cfg.public_b_chunk_atoms = 1;
    MarketGroupV13::new(market, cfg).unwrap()
}

fn fuzz_accounts() -> (PortfolioAccountV13, PortfolioAccountV13) {
    let (market, a_id, b_id, owner) = ids();
    (
        PortfolioAccountV13::empty(ProvenanceHeaderV13::new(market, a_id, owner)),
        PortfolioAccountV13::empty(ProvenanceHeaderV13::new(market, b_id, owner)),
    )
}

fn prices(price: u64) -> [u64; V13_MAX_PORTFOLIO_ASSETS_N] {
    [price; V13_MAX_PORTFOLIO_ASSETS_N]
}

fn assert_fuzz_invariants(
    group: &MarketGroupV13,
    a: &PortfolioAccountV13,
    b: &PortfolioAccountV13,
) {
...
fn run_with_svm_rollback(
    ...
    a: &mut PortfolioAccountV13,
    b: &mut PortfolioAccountV13,
    result: Result<(), V13Error>,
    before: (MarketGroupV13, PortfolioAccountV13, PortfolioAccountV13),
) {
    if result.is_err() {
        *group = before.0;
        *a = before.1;
...
proptest! {
    ...
    #[test]
    fn v13_fuzz_public_live_actions_preserve_conservation_under_svm_rollback(
