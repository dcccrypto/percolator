...
// E1 (Finding C / toly 7188eec): the SAME wind-down, but the backing bucket is now
// TIME-EXPIRED at resolution (resolved_slot 2 > bucket expiry 1). The expiry-agnostic
// terminal release must STILL unwind the counterparty lien (not revert via the freshness
// guard), closing the residual Finding-C deadlock. Operative: pnl->0 + lien->0 must hold.
#[kani::proof]
#[kani::unwind(40)]
#[kani::solver(cadical)]
