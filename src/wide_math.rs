...
// Clippy: shl/shr/bitand/bitor are intentionally named to mirror operator
// trait methods; we implement the actual traits below but keep the plain
// methods for internal call-sites in non-operator contexts.
...
pub fn mul_div_ceil_u128(a: u128, b: u128, d: u128) -> u128 {
    ...
    let q = p / d;
    if !p.is_multiple_of(d) { q + 1 } else { q }
}
